...
enum {INVALID = 0, PAUSED, RUNNING, FINISHED, CREATING};
...
ucontext_t threads_data[MAX_THREADS];
...
__thread ucontext_t parent; // main thread

__thread int current_thread = 0;
...
*/
void create_thread_make_content_helper(void (*fun_ptr)(), void* parameter){
...
void create_new_parameterized_thread(void (*fun_ptr)(void*), void* parameter) {
    void(*cast_ptr)() = (void(*)()) create_thread_make_content_helper;
    ...
    for(int i=0; i<MAX_THREADS; i++){
        if(thread_state[i] == INVALID){
            ...
            thread_state[i] = CREATING;

            getcontext( &threads_data[i]);
            threads_data[i].uc_link = 0;
            threads_data[i].uc_stack.ss_sp = malloc( THREAD_STACK_SIZE );
            threads_data[i].uc_stack.ss_size = THREAD_STACK_SIZE;
            threads_data[i].uc_stack.ss_flags = 0;

            if ( threads_data[i].uc_stack.ss_sp == 0 ) {
               perror( "malloc: Could not allocate stack\n" );
               exit (1);
            }

            makecontext(&threads_data[i], cast_ptr, 2, fun_ptr, parameter);
...
void* schedule_threads_pthread(void* args){
    int thread_num = 0;
        ...
        for(int i=0; i < MAX_THREADS; i++){
            ...
            switch (thread_state[i])
            {
               case PAUSED:
                  ...
                  swapcontext(&parent, &threads_data[current_thread]);
                  break;

               case FINISHED:
                  thread_state[i] = INVALID;
                  pthread_mutex_unlock(&mutex);
                  free(threads_data[i].uc_stack.ss_sp);
...
void schedule_hybrid_threads(int num_pthreads){
   int i;
   pthread_t my_pthreads[num_pthreads];
   for(i=0;i<num_pthreads;i++){
      pthread_create(&my_pthreads[i], NULL, schedule_threads_pthread, NULL);
   }

   for(i=0;i<num_pthreads;i++){
      pthread_join(my_pthreads[i], NULL);
   }
...
*/
void yield() {
   thread_state[current_thread] = PAUSED;
   swapcontext( &threads_data[current_thread], &parent);
}
...
*/
void finish_thread() {
   thread_state[current_thread] = FINISHED;
	swapcontext( &threads_data[current_thread], &parent);
}
