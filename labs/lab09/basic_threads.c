...
int thread_cnt = 0; // count thread number
ucontext_t parent; // main thread
int current_thread = 0; // count current thread
bool threads_finish[MAX_THREADS]; // store thread is done or not
int toal_threads = 0;
...
void create_thread_helper(void (*fun_ptr)(), int i, void* parameter){
	...
	makecontext(&threads[i], (void *)create_thread_make_content_helper, 2, fun_ptr, parameter);
}
...
void create_new_parameterized_thread(void (*fun_ptr)(void*), void* parameter) {
	...
	if(thread_cnt <MAX_THREADS){
		...
		create_thread_helper(fun_ptr, thread_cnt, parameter);
		thread_cnt ++;
		...
		for(int i=0;i<MAX_THREADS; i++){
			if(threads_finish[i]){
				threads_finish[i] = false;
				create_thread_helper(fun_ptr, i, parameter);
				break;
			}
		}
	}
...
void schedule_threads() {
	int j = 0;
	while(j < toal_threads){
		
		for(int i=0;i<thread_cnt;i++){
			current_thread = i;
			bool flag = false;
			if(!threads_finish[current_thread]){
				swapcontext(&parent, &threads[current_thread]);
				flag = true;
			}
			
			if(threads_finish[current_thread] && flag){
				j ++;
...
*/
void finish_thread() {
	threads_finish[current_thread] = true;
	swapcontext( &threads[current_thread], &parent);
}
